...
export default function AlertBanner({
  ...
  const inner = (
    <span className="inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm sm:text-base lg:text-lg font-bold">
      <span aria-hidden="true" className="hidden sm:inline">{icon}</span>
      <span className="text-center leading-tight">{t.message}</span>
      {t.linkLabel && (
        <span className="ml-1 inline-flex items-center gap-1.5 rounded-full bg-blue-600 px-4 py-1 text-sm font-bold text-white shadow-md shadow-blue-900/20 transition-all hover:scale-105 hover:bg-blue-700 whitespace-nowrap">
          {t.linkLabel}
